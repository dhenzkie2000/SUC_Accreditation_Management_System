    ...
    if(!empty($_POST['up_param_code']) || !empty($_POST['up_param_desc'])){
        ...
        if(!isset($_POST['support_doc'])){
            if($up_param_code != $up_old_param_code){
                ...
                $check_query = "SELECT * FROM accsys.maintenance_area_instrument WHERE code = '$up_param_code' AND area_param = '$parameter_code'";
                ...
                if(pg_num_rows($check_result) > 0){
                    ...
                    $up_query = "UPDATE accsys.maintenance_area_instrument
                    SET
                    code = '$up_param_code',
                    ...
                    if(pg_affected_rows($up_result) > 0){
                        ...
                        foreach($_POST['support_doc_name'] as $key => $value)
                        {
                            if(in_array($_POST['support_doc_name'][$key], $checked_array)){
                                ...
                                $query = "INSERT INTO accsys.maintenance_area_instrument_docs (instrument_keyctr, scode, offccode, trail)
